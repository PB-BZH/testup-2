  ...
  def self.init_ui
    ...
    if defined?(Sketchup)
      cmd.set_validation_proc {
        window&.visible? ? MF_CHECKED : MF_ENABLED
      }
    end
    ...
    if defined?(Sketchup)
      cmd.set_validation_proc {
        settings[:run_in_gui] ? MF_ENABLED : MF_CHECKED
      }
    end
    ...
    if defined?(Sketchup)
      cmd.set_validation_proc {
        settings[:seed] ? MF_ENABLED : MF_CHECKED
      }
    end
    cmd_seed = cmd
    ...
    if defined?(Sketchup)
      cmd.set_validation_proc {
        flags = 0
        flags |= MF_GRAYED if settings[:run_in_gui]
        flags |= MF_CHECKED if settings[:verbose_console_tests]
        flags
      }
    end
    ...
    if defined?(Sketchup)
      ...
      sub_menu.set_validation_proc(menu_id) do
  ...
  module SystemUI
    def self.select_directory(options)
      if defined?(UI) && UI.respond_to?(:select_directory)
        ...
        select_directory_fallback(options)
      end
    end

...
module SKUI::Examples
  def self.show
    ...
    window.on(:resize) do |window, width, height|
    ...
    lst_dropdown.on(:change) do |control, value| # (?) Second argument needed?
    ...
    lst_list.width = 130
    lst_list.height = 90
    lst_list.on(:change) do |control, value|

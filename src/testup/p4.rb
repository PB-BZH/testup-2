  ...
  module Perforce
    ...
    def self.find_p4_paths
      ...
      search_locations.each do |location|
        client_roots.each do |client_root|
    ...
    def self.find_trunk_client
      trunks = find_p4_paths.grep(/trunk/i)
    ...
    def self.replace_sketchup_testsuites(client_path)
      testsuites = find_sketchup_tests(client_path)
      ...
      other_tests = TestUp.settings[:paths_to_testsuites].reject { |path|
        ...
        expected_tests.include?(testsuite_name)
      }
      # Append the new.
